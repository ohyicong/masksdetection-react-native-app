  ...
  class MathBackendWebGL extends tf.KernelBackend {
      ...
      add(a, b) {
          ...
          if (this.shouldExecuteOnCPU([a, b])) {
              const aData = this.texData.get(a.dataId);
              const bData = this.texData.get(b.dataId);
              const [outValues, outShape] = addImplCPU(a.shape, b.shape, aData.values, bData.values, dtype);
              return this.makeOutput(outShape, dtype, outValues);
          }
          if (tf.env().getBool('WEBGL_PACK_BINARY_OPERATIONS')) {
      ...
      subtract(a, b) {
          ...
          if (this.shouldExecuteOnCPU([a, b])) {
              const aData = this.texData.get(a.dataId);
              const bData = this.texData.get(b.dataId);
              const [outValues, outShape] = subImplCPU(a.shape, b.shape, aData.values, bData.values, dtype);
              return this.makeOutput(outShape, dtype, outValues);
          }
          if (tf.env().getBool('WEBGL_PACK_BINARY_OPERATIONS')) {
      ...
      conv2dByMatMul(x, filter, convInfo, bias, activation, preluActivationWeights) {
          ...
          if (batchMatMulWillBeUnpacked || !tf.env().getBool('WEBGL_LAZILY_UNPACK') ||
              ...
              const xReshaped = tf.reshape(x, [1, targetShape, convInfo.inChannels]);
              const filterReshaped = tf.reshape(filter, [1, convInfo.inChannels, convInfo.outChannels]);
...
`;
  ...
  return result;
`;
  const atan2KernelFunc = binaryKernelFunc(ATAN2, ATAN2_PACKED);
  const atan2Config = {
      kernelName: tf.Atan2,
      backendName: 'webgl',
   ...
   * =============================================================================
   */
  const batchNormKernelFunc = ({ inputs, backend, attrs }) => {
  ...
  return cos(x);
`;
  const cosKernelFunc = unaryKernelFunc(COS);
  const cosConfig = {
      kernelName: tf.Cos,
      backendName: 'webgl',
...
return a / b;`;
  ...
  return result;
`;
  const divKernelFunc = binaryKernelFunc(DIV, DIV_PACKED, true /* checkOutOfBoundsForPackedProgram */);
  const divConfig = {
      kernelName: tf.Div,
      backendName: 'webgl',
  ...
  return sin(x);
`;
  const sinKernelFunc = unaryKernelFunc(SIN);
  const sinConfig = {
      kernelName: tf.Sin,
      backendName: 'webgl',
  ...
  const squareKernelFunc = unaryKernelFunc(SQUARE);
  const squareConfig = {
  ...
  const squaredDifferenceKernelFunc = binaryKernelFunc(SQUARED_DIFFERENCE, SQUARED_DIFFERENCE);
  ...
  const tanKernelFunc = unaryKernelFunc(TAN);
  const tanConfig = {
      kernelName: tf.Tan,
      backendName: 'webgl',
